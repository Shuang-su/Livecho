...
| `ASSET-SAFETY` | Orthogonal global enable state and canonical-room denylist under one monotonic safety generation, recovery-visible serving-incarnation continuity guard, prepared relaxation proposals, non-restorable current-incarnation global activation and one-use room-removal permit, owner approvals, and emergency-disable result | Recovery records are integrity protected, rollback resistant, held outside application backups, and default off. A prepared relaxation is not current; a committed enabled snapshot alone cannot admit traffic; a removal proposal cannot drop membership before its same-incarnation final guard atomically permits promotion; and an orphaned/unsealed continuity guard quarantines every traffic class. Global transitions affect all rooms while a successful room transition affects only its canonical target. |
...
| `THREAT-RESTORE-RESURRECTION` | `ASSET-DELETION`, `ASSET-SAFETY`, `ASSET-IDENTITY`, `ASSET-SECRET`, all persisted data | `ACT-PROVIDER`, mistaken `ACT-PRIVILEGED`, stale backup | `ENTRY-DELETION-RESTORE`, `ENTRY-MAINTENANCE` | Restart/restore treats an empty application store/audit as proof, trusts a prepared/ambiguous relaxation or committed enabled snapshot without a fresh current-incarnation activation capability, restores a guard-failed or pre-dispatch room removal as an absent entry, guesses an unknown post-dispatch result, or reuses its prior-incarnation permit; seals or ignores an orphaned continuity guard; opens with unresolved or exact-intake-lost deletion scope; fails room/session or account/device replay; loses room dominance; accepts a conflicting parent map or pre-restore credential/key; or uses stale safety scope or an unbounded backup/object window. | `CTRL-SAFETY-DEFAULT-OFF`, `CTRL-RESTORE-REPLAY`, `CTRL-DELETION-STATE`, `CTRL-IDENTITY-RESTORE-REVOCATION`, `CTRL-AUTH-KEY-LIFECYCLE`, `CTRL-BACKUP-EVIDENCE`, `CTRL-MAINTENANCE-EXCLUSIVE`; process-bound non-restorable global activation and removal permit, continuity-guard quarantine, durable admission, and provider-window evidence. | Orphaned/unsealed-guard and exact-intake-loss restart negatives; global-enable prepared/post-promotion/pre-activation and committed-without-capability crash probes; room-removal final-guard/permit failure, crash before dispatch, crash after dispatch with unknown outcome, and post-promotion response-loss probes; unresolved-intake and missing-tombstone restore negatives; offline credential/new-authority rejection; room/session/sibling/dominance reconciliation; auth and safety-snapshot generation comparison; post-window verification. | Keep every traffic class isolated/off; discard or reconcile prepared/split global enable and never reuse a prior incarnation's activation; preserve room membership after every failed pre-dispatch removal and reject every prior-incarnation permit; after dispatch require exact journal/head reconciliation to accept only the matching permitted `COMMITTED` removal, retain the room for unchanged predecessor/`PREPARED`, and remain off for split/unknown state; require authoritative ingress continuity before sealing an orphaned guard; reconcile every exact intake to a commit/read-back-verified tombstone before replay; start no guessed purge; reject restored credentials, advance protected auth state, replay exact-scope cascades/checkpoints, and require fresh non-restored audited recovery-admin auth. | Identity/data/operations owners; Issues #4/#12/#13/#16/#19 | **High**: an omitted/unadmitted/wrong-scope target, orphaned authority epoch, stale enabled snapshot, failed removal restored as absent, or accepted pre-restore credential can republish data, delete unrelated data, or recreate authority. | **NOT ACCEPTED**; auth/persistence/recovery production paths OFF; see decision register. |
