...
| `CTRL-SAFETY-GENERATION` | One monotonic generation orders an append-only journal and integrity-protected recovery snapshot containing an orthogonal global enable bit and complete canonical-room denylist. Enable/remove first writes and reads back a predecessor-bound `PREPARED` proposal under an opaque proposal identity; preparation is not current and does not advance the generation or authorize traffic/recovery. Only successful conditional promotion to `COMMITTED` advances the one generation. A `COMMITTED` enabled snapshot still requires the same live incarnation's non-restorable activation capability, minted only by the final local fence check-and-effect. A high-priority tightening fence marks the serving-continuity epoch safety-pending and preempts slow relaxation I/O: global disable revokes global activation, while `add(R)` installs only `R`'s block/removal effect and preserves unrelated-room activation. Prepared, split, ambiguous, post-promotion/pre-activation, or unreconciled tightening outcomes remain off/quarantined and prevent clean sealing. A generation change reloads/re-evaluates state; it does not itself imply global cleanup. | Issues #4, #7, #16, #17, #19 |
...
| `CTRL-DISABLE-CLEANUP` | Apply cleanup at the verified effective scope: Alpha uses one active serving authority process, and global disable first revokes its live activation capability, closes its gates, and starts every active/queued-room cleanup before any journal/recovery-copy await, so slow, hung, or failed durable I/O cannot extend disclosure; a successful denylist add rejects/stops only matching-room starts/reconnects, platform session, lease, conforming audio/locator RAM, and pending publication, with unrelated-room noninterference. Future multi-owner fence/ack uncertainty isolates the owner and fails globally. | Issues #7, #8, #11, #14, #15, #16, #17, #19 |
| `CTRL-RESTORE-REPLAY` | Reject restored stateful/stateless credentials; reject every prepared/ambiguous relaxation and any committed enable lacking a freshly reconciled current-incarnation activation capability; quarantine an orphaned/unsealed serving guard; reconcile every continuity failure against authoritative ingress replay and every unresolved deletion intake/admission to a verified replayable tombstone; replay current deletion/revocation checkpoints; and reconcile the complete global/denylist snapshot before accepting any authentication, viewer/history/publication, ingest, worker, export, persistence, callback, or maintenance traffic. | Issues #4, #12, #13, #16, #19 |
...
| `CTRL-DELETION-FAIL-CLOSED` | Before verified admission, containment is provisional and no deletion state/success is reported or purge started. When the backend observes an authenticated valid selector it owns a sticky unresolved hazard and retries exact intake; the initiating source is not the sole replay owner. If exact intake durability fails, the already-open recovery-visible serving guard cannot be cleanly sealed. Initiating-source loss plus backend crash therefore leaves an orphaned guard that globally quarantines every data surface until authoritative replay recovers the exact triple; if it cannot, quarantine persists. The guard is not `hidden`, a fourth deletion state, an SLA clock, or purge authority. After admission, any failed or unchecked store stays `hidden` and cannot be reported complete during idempotent retry. Volatile containment, an audit row, or an empty store is not a tombstone. | Issues #11, #16, #17 |
...
| `THREAT-SAFETY-ROLLBACK` | `ASSET-SAFETY`, `ASSET-AVAILABILITY` | Compromised/mistaken `ACT-PRIVILEGED`, stale backup, storage failure | `ENTRY-BROWSER` admin control, `ENTRY-DATA`, `ENTRY-DELETION-RESTORE` | A newer global/room decision is overwritten; global cleanup waits behind slow/hung durable I/O; a relaxation is made recovery-current before its final fence, or a committed enabled snapshot is treated as sufficient after the final fence fails; tightening exists only in volatile state when the process crashes; a delayed enable/remove response clears a newer local latch/block; multiple active owners do not share/acknowledge the fence; a crash, split journal/recovery commit, or lost response drops scope/state; clean-close races a late safety action; a room add is misapplied globally or fails to stop its matching room; enable drops denylist entries or removal enables global ingest; canonical/resource binding is ambiguous; state is stale/lost; or relaxation lacks owner approval. | `CTRL-SAFETY-DEFAULT-OFF`, `CTRL-SAFETY-GENERATION`, `CTRL-ROOM-DENYLIST`, `CTRL-DISABLE-CLEANUP`, `CTRL-REENABLE-GATE`; one active Alpha serving authority, pre-armed continuity guard, process-bound non-restorable activation capability, non-current predecessor-bound prepared relaxation, future-owner isolation on fence uncertainty, high-priority tightening fence, pre-durability local global-cleanup start, one-generation orthogonal snapshot, predecessor binding, and exact-scope effects. | Slow/hung/write-failed safety-I/O injection with active-room cleanup observation; active-owner enumeration/fence acknowledgement; prepared/current/capability inspection; both linearization orders for relaxation versus tightening and disable versus add; final-fence failure after candidate durability followed by tightening-write failure and process crash; clean-close versus disable/add in both orders with failure/exit; pre-/mid-/post-promotion and pre-/post-activation crash plus response-loss replay; orphaned-guard recovery; generation/recovery mismatch; matching- and unrelated-room transition probes; global-enable/room-remove noninterference tests; stale/concurrent operation, canonicalization/binding, state-age, and integrity alerts. | Apply exact verified scope without awaiting durability: global disable first revokes global activation and starts all-room cleanup; `add(R)` first installs only `R`'s block and starts `R` cleanup while preserving unrelated-room activation. A prepared relaxation is never current or recoverable, and a committed candidate without the same live incarnation's final capability admits no traffic. If tightening durability then process continuity are lost, the orphaned guard globally quarantines recovery rather than honoring the candidate. Isolate/terminate any unacknowledged future owner; reconcile split/unknown proposals and exact committed decisions without dropping the denylist. Any scope/binding/commit/guard ambiguity invokes local global forced-off, expands cleanup globally, and denies relaxation. | Safety/security/operations owners; Issues #7/#16/#17/#19 | **High**: delayed cleanup, stale relaxation, uncoordinated authority, lost/split state, control compromise, or wrong scope can expose denied content or cause broad availability loss. | **NOT ACCEPTED**; production ingest OFF; see decision register. |
...
1. Emergency global disable is available to operator/admin independently of the failing
   ...
   any future owner that cannot
   synchronously acknowledge the fence is isolated/terminated and keeps the deployment
   globally off. A successfully committed
...
2. One generation orders an orthogonal global-enable bit and complete denylist. Global
   ...
   fence check-and-effect. Global disable preempts slow relaxation I/O by revoking that
   capability; `add(R)` preempts only the matching room-removal effect and keeps unrelated
   rooms' global activation. A
   prepared, split, ambiguous, post-promotion/pre-activation, or fence-failed result stays
   off; after process loss an unsealed/orphaned continuity guard prevents a committed
   candidate from reopening. Canonical/resource ambiguity, stale/conflicting state,
   journal/recovery failure, unknown rights scope, or unauthorized relaxation invokes
   global off/quarantine.
3. A deletion request first validates exactly one authoritative room-or-session selector,
   then establishes provisional containment and a sticky backend-owned unresolved hazard
   while atomically retaining selector, idempotency, and immutable original initiating-
   request time in durable intake. The initiating source is not the sole replay owner. The
   existing `hidden` tombstone must reuse that triple and commit/read back at the
   independent recovery boundary. Before that barrier, no request is accepted/
   acknowledged, no deletion state is reported, and no purge begins; an audit event,
   volatile block, coarse guard, or empty store is not a substitute. If exact intake
   persistence fails, the pre-armed serving-incarnation guard cannot be cleanly sealed.
   Loss of both source and backend therefore leaves recovery globally quarantined until
   authoritative replay recovers the exact triple; absent that proof, quarantine persists
   without inventing `hidden`, a fourth state, a later SLA clock, or destructive scope. A
   valid durable intake survives for retry, while a post-commit/pre-response crash or lost
   response reuses the same tombstone, identity, and original time.
...
- deletion tests prove the recovery-visible serving guard is durably open before traffic;
  inject exact-intake write/read-back failure followed by initiating-source loss and
  backend crash; prove the live backend owns a sticky retry, clean sealing is impossible,
  restart detects the orphaned guard, every data surface and relaxation remains
...
- safety tests distinguish global disable from matching and unrelated-room denylist adds,
  prove local authority closure and global cleanup actions precede every durable-I/O
  await under slow, hung, timeout, and write/read-back failure injection; prove a
  ...
  and escalate canonical/resource-binding or journal/recovery ambiguity globally;
