...
| `CTRL-SAFETY-GENERATION` | One monotonic generation orders an append-only journal and integrity-protected recovery snapshot containing an orthogonal global enable bit and complete canonical-room denylist. A high-priority pending-tightening fence lets disable/add preempt slow relaxation I/O; the final relaxation fence check and local effect are one atomic compare-and-set. A generation change reloads/re-evaluates state; it does not itself imply global cleanup. | Issues #4, #7, #16, #17, #19 |
...
| ID | Asset | Actor | Entry point | Preconditions | Preventive controls | Detection | Response | Control owner / dependency | Residual | Decision |
| --- | --- | --- | --- | --- | --- | --- | --- | --- | --- | --- |
| `THREAT-DELETION-INCOMPLETE` | `ASSET-DELETION`, `ASSET-NORMALIZED`, `ASSET-RAW`, `ASSET-IDENTITY` | `ACT-PROVIDER`, coding error, or `ACT-PRIVILEGED` | `ENTRY-DATA`, `ENTRY-DELETION-RESTORE` | A request is acknowledged, `hidden` reported, or purge started before tombstone commit/read-back; durable intake omits/resets the original request time; a pre/post-commit crash or lost response loses/duplicates admission; an audit row, volatile block, or empty store is mistaken for a tombstone; a selector is invalid/conflicting; room/session scope or dominance is wrong; a cascade fails; or late success erases the SLA breach. | `CTRL-DELETION-STATE`, `CTRL-DELETION-FAIL-CLOSED`, `CTRL-IDENTITY-RESTORE-REVOCATION`, `CTRL-BACKUP-EVIDENCE`, `CTRL-ROOM-DENYLIST`; atomic durable intake triple, verified `hidden` admission barrier, authoritative selector, room dominance, exact-scope cascade, and checkpoint read-back. | Intake/tombstone triple and immutable-clock comparison; admission commit/read-back fault injection; pre-commit and post-commit/pre-response crash plus response-loss/idempotent-retry tests; audit/empty-store negative probes; selector, room-all-child, sibling non-deletion, dominance, store, identity, retry, and SLA reconciliation. | Keep provisional containment and global off before verified admission; report no success/state and start no purge; reconcile unresolved intake to the same tombstone/idempotency/original-time triple; after admission retry/re-enumerate exactly, never report unchecked completion, and preserve dominance and late-SLA evidence. | Identity/data/privacy/operations owners; Issues #12/#13/#16 | **High**: lost admission/time, wrong scope, provider failure, an omitted authority row, or an unknown copy may expose retained data, hide lateness, or delete unrelated data. | **NOT ACCEPTED**; production auth/persistence OFF; see decision register. |
...
| `THREAT-SAFETY-ROLLBACK` | `ASSET-SAFETY`, `ASSET-AVAILABILITY` | Compromised/mistaken `ACT-PRIVILEGED`, stale backup, storage failure | `ENTRY-BROWSER` admin control, `ENTRY-DATA`, `ENTRY-DELETION-RESTORE` | A newer global/room decision is overwritten; global cleanup waits behind slow/hung durable I/O; a delayed enable/remove response clears a newer local latch/block; multiple active owners do not share/acknowledge the fence; a crash, split journal/recovery commit, or lost response drops scope/state; a room add is misapplied globally or fails to stop its matching room; enable drops denylist entries or removal enables global ingest; canonical/resource binding is ambiguous; state is stale/lost; or relaxation lacks owner approval. | `CTRL-SAFETY-DEFAULT-OFF`, `CTRL-SAFETY-GENERATION`, `CTRL-ROOM-DENYLIST`, `CTRL-DISABLE-CLEANUP`, `CTRL-REENABLE-GATE`; one active Alpha serving authority, future-owner isolation on fence uncertainty, high-priority tightening fence, atomic final relaxation check-and-effect, pre-durability local global-cleanup start, one-generation orthogonal snapshot, predecessor binding, and exact-scope effects. | Slow/hung/write-failed safety-I/O injection with active-room cleanup observation; active-owner enumeration/fence acknowledgement; both linearization orders for relaxation versus tightening and disable versus add; pre-/mid-/post-commit crash and response-loss replay; generation/recovery mismatch; matching- and unrelated-room transition probes; global-enable/room-remove noninterference tests; stale/concurrent operation, canonicalization/binding, state-age, and integrity alerts. | Apply exact verified scope without awaiting durability: successful room add cleans only its room; global disable starts cleanup for all active/queued rooms before durable I/O. A late relaxation cannot clear a newer fence; either relaxation linearizes first and the later tightening closes it, or tightening linearizes first and relaxation fails without reopening. Isolate/terminate any unacknowledged future owner; restart default-off; reconcile split/unknown commits and reuse exact committed decisions without dropping the denylist. Any scope/binding/commit ambiguity invokes local global forced-off, expands cleanup globally, denies relaxation, and reconciles the complete snapshot before re-enable. | Safety/security/operations owners; Issues #7/#16/#17/#19 | **High**: delayed cleanup, stale relaxation, uncoordinated authority, lost/split state, control compromise, or wrong scope can expose denied content or cause broad availability loss. | **NOT ACCEPTED**; production ingest OFF; see decision register. |
...
1. Emergency global disable is available to operator/admin independently of the failing
   ingest path, immediately rejects every room, closes its local authority gates, and
   issues all active/queued cleanup before any journal/recovery-copy await. Slow, hung, or
   failed durable I/O cannot defer that cleanup. Alpha has one active serving authority;
...
2. One generation orders an orthogonal global-enable bit and complete denylist. Global
   ...
   Tightening preempts slow relaxation I/O; the final relaxation fence check and effect
   are atomic, so neither race order permits a late enable/remove to clear a newer block.
3. A deletion request first validates exactly one authoritative room-or-session selector,
   then establishes provisional containment and atomically retains selector, idempotency,
   and immutable original initiating-request time in durable intake. The existing
   `hidden` tombstone must reuse that triple and commit/read back at the independent
   recovery boundary. Before that barrier, no
...
- deletion tests inject commit/read-back failure, pre-commit crash, post-commit/pre-
  ...
  barrier; prove intake atomically retains selector/idempotency/original time before a
  pre-tombstone crash and that retry/tombstone reuse the unchanged triple; reject audit,
...
- safety tests distinguish global disable from matching and unrelated-room denylist adds,
  ...
  await under slow, hung, timeout, and write/read-back failure injection; inject disable
  versus enable and add versus
  remove on both sides of the atomic final check-and-effect and prove no reopen window;
