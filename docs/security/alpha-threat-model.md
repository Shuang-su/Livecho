...
| `ASSET-DELETION` | Exactly-one typed room-or-session deletion manifests, typed pseudonymous account/device deletion/revocation checkpoints, active-purge state, backup-window state, and SLA-breach result | Exact-scope, idempotent, and replayable before restored traffic; encrypted, narrowly authorized, access-audited, and protected against ambiguity, rollback, or omission. |
...
| `ENTRY-DELETION-RESTORE` | Typed room/session selector parsing and authoritative resolution, takedown, deletion retry, identity/device revocation, bearer invalidation, checkpoint replay, application backup, restore, and re-enable process. |
...
| `CTRL-RESTORE-REPLAY` | Reject restored stateful/stateless credentials, replay current deletion/revocation checkpoints, and reconcile safety generation/denylist before accepting restored authentication, ingest, worker, or viewer traffic. | Issues #4, #12, #13, #16, #19 |
...
| `CTRL-DELETION-STATE` | Require exactly one selector: canonical room scope covers room metadata and all child sessions, while authoritative immutable-session scope covers only that session; room tombstones dominate child manifests; ambiguity blocks safely without guessed purge; exact-scope cascade reports three truthful states with immutable late-SLA evidence. | Issue #16 |
| `CTRL-BACKUP-EVIDENCE` | Enumerate and evidence every cache, replica, object version, export, and backup window; retain room/session and identity/device checkpoints plus independent auth-invalidation state through every window and one verified post-window restore; never claim physical erasure. | Issues #4, #12, #13, #16, #19 |
...
| ID | Asset | Actor | Entry point | Preconditions | Preventive controls | Detection | Response | Control owner / dependency | Residual | Decision |
| --- | --- | --- | --- | --- | --- | --- | --- | --- | --- | --- |
| `THREAT-DELETION-INCOMPLETE` | `ASSET-DELETION`, `ASSET-NORMALIZED`, `ASSET-RAW`, `ASSET-IDENTITY` | `ACT-PROVIDER`, coding error, or `ACT-PRIVILEGED` | `ENTRY-DATA`, `ENTRY-DELETION-RESTORE` | A selector is empty/composite/ambiguous or trusts a conflicting parent hint; room scope misses a pending/late/restored child; session scope over-deletes siblings/shared state or misses a derivative; a narrower tombstone overwrites room scope; a store/identity cascade fails; or late success erases the SLA breach. | `CTRL-DELETION-STATE`, `CTRL-DELETION-FAIL-CLOSED`, `CTRL-IDENTITY-RESTORE-REVOCATION`, `CTRL-BACKUP-EVIDENCE`, `CTRL-ROOM-DENYLIST`; authoritative exactly-one selector, room dominance, exact-scope cascade, and durable checkpoint read-back. | Separate room/session selector tests; invalid/conflicting-selector rejection; room-all-child, sibling non-deletion/shared-state, dominance, enumerated-store, and identity-authority reconciliation; payload-free manifest integrity; retry/SLA alerts. | Block the widest safely identified exposure; start no guessed purge; preserve room dominance; retry/re-enumerate idempotently; never report completion while scope/store is unchecked; preserve permanent deletion and late-SLA state; incident escalate. | Identity/data/privacy/operations owners; Issues #12/#13/#16 | **High**: wrong scope, provider failure, an omitted authority row, or an unknown copy may expose retained data or delete unrelated data. | **NOT ACCEPTED**; production auth/persistence OFF; see decision register. |
| `THREAT-RESTORE-RESURRECTION` | `ASSET-DELETION`, `ASSET-SAFETY`, `ASSET-IDENTITY`, `ASSET-SECRET`, all persisted data | `ACT-PROVIDER`, mistaken `ACT-PRIVILEGED`, stale backup | `ENTRY-DELETION-RESTORE`, `ENTRY-MAINTENANCE` | Restore opens traffic before room-all-session/exact-session and account/device checkpoints replay, loses room dominance or accepts a conflicting parent map, accepts backed-up stateful/stateless credential/key state, or uses stale safety/denylist state or an unbounded backup/object window. | `CTRL-SAFETY-DEFAULT-OFF`, `CTRL-RESTORE-REPLAY`, `CTRL-DELETION-STATE`, `CTRL-IDENTITY-RESTORE-REVOCATION`, `CTRL-AUTH-KEY-LIFECYCLE`, `CTRL-BACKUP-EVIDENCE`, `CTRL-MAINTENANCE-EXCLUSIVE`; provider-window evidence. | Offline credential and new-authority rejection; restored room-all-child/exact-session/sibling/dominance reconciliation; auth/safety generation or key-version comparison; restore tabletop and post-window verification. | Keep isolated/off; start no guessed purge on map conflict; purge restored verifier/session rows; advance protected auth state; replay exact-scope cascades/checkpoints with room dominance; use fresh non-restored audited recovery-admin auth; destroy unsafe restore if needed. | Identity/data/operations owners; Issues #4/#12/#13/#16/#19 | **High**: an omitted/wrong-scope target or accepted pre-restore credential can republish data, delete unrelated data, or recreate authority. | **NOT ACCEPTED**; auth/persistence/recovery production paths OFF; see decision register. |
...
| `THREAT-DELETION-INCOMPLETE` | Production normalized/raw/identity persistence and managed exports subject to room/session/account/device deletion. | High | `CTRL-DELETION-STATE`, `CTRL-IDENTITY-RESTORE-REVOCATION`, `CTRL-BACKUP-EVIDENCE`, provider inventory/window evidence, invalid-selector, room-all-child, session-sibling, dominance, cascade/read-back, and partial-failure tests. | Admin and identity/data/privacy owner | **NOT ACCEPTED — approver/date/review date PENDING; auth/persistence OFF** |
| `THREAT-RESTORE-RESURRECTION` | Restoring any production data backup or object-version state, including exact deletion scope, identity, and stateful/stateless credential state. | High | `CTRL-SAFETY-DEFAULT-OFF`, `CTRL-RESTORE-REPLAY`, `CTRL-DELETION-STATE`, `CTRL-IDENTITY-RESTORE-REVOCATION`, `CTRL-AUTH-KEY-LIFECYCLE`, `CTRL-BACKUP-EVIDENCE`, exact-scope/dominance restore, credential-rejection, and deleted-authority issuance probes. | Identity/operations/data owner | **NOT ACCEPTED — approver/date/review date PENDING; auth/recovery path OFF** |
...
3. A deletion request validates exactly one authoritative room-or-session selector before
   destructive work. Room scope blocks and re-enumerates every child session; session
   scope removes only its target/derivatives and preserves siblings/shared state; room
   tombstones dominate child manifests. Ambiguity blocks safely without guessed purge.
   Partial purge never reports completion, retries idempotently, and preserves immutable
   late-SLA evidence.
4. Restore remains offline and globally disabled until every restored stateful credential
   row is purged/revoked, protected auth generation/key state rejects every stateless
   pre-restore credential, typed room-all-session/exact-session and account/device
   deletion/revocation checkpoints replay with dominance and exact-scope proofs, and
   current safety/denylist state reconciles. A backup cannot
   restore current verification key material, overwrite the separate recovery copy, or
   recreate deleted authority.
...
6. Re-enable requires cause remediation, current platform/rights evidence, deletion
   replay where relevant, successful tabletop evidence, recorded repository-owner
   approval, and fresh non-restored separately audited admin recovery authentication.
   Availability pressure and restored admin sessions are not exceptions.
...
- secret redaction, protocol rejection, no-audio-persistence, raw archival failure,
  platform fail-closed, invalid room/session selector rejection, room-all-child and
  session-sibling exact-scope proofs, room-tombstone dominance, deletion partial failure,
  stateful/stateless pre-restore credential rejection, account/device cascade and new-
  authority denial, identity/device checkpoint replay, and fresh recovery-admin paths
  have negative tests or tabletop evidence appropriate to their Issue;
