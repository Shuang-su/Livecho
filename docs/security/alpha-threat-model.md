...
| `ASSET-SAFETY` | Orthogonal global enable state and canonical-room denylist under one monotonic safety generation, recovery-visible serving-incarnation continuity guard, prepared relaxation proposals, non-restorable current-incarnation global activation and one-use room-removal permit, owner approvals, and emergency-disable result | Recovery records are integrity protected, rollback resistant, held outside application backups, and default off. A prepared relaxation is not current; a committed enabled snapshot alone cannot admit traffic; a removal proposal cannot drop membership before its same-incarnation final guard atomically permits promotion; and an orphaned/unsealed continuity guard quarantines every ordinary/external/scheduled traffic class and credential issuance while permitting only the mutually exclusive offline recovery actor to reconcile a trustworthy intact epoch. Global transitions affect all rooms while a successful room transition affects only its canonical target. |
...
| `ASSET-DELETION` | Durable deletion/revocation intake with immutable original initiating-request time, exactly-one typed room-or-session `hidden` tombstones, unresolved admission blockers, typed pseudonymous account/device deletion/revocation checkpoints, durable invalid denials, active-purge state, backup-window state, and SLA-breach result | Exact-scope, idempotent, commit/read-back verified before acknowledgement or purge, and replayable before restored traffic or credential issuance; encrypted, narrowly authorized, access-audited, and protected against clock reset, crash, response loss, ambiguity, rollback, or omission. A pre-armed recovery-visible continuity guard covers room/session and account/device ingress and preserves a coarse no-reopen fact if exact outcome durability and both initiating-source/backend continuity are lost; it is not a tombstone, identity checkpoint, denial, deletion state, SLA clock, or purge authority. |
...
| `CTRL-SAFETY-DEFAULT-OFF` | Start every process and restore globally disabled with no reusable activation capability, ignoring backed-up enable state. Before any traffic, durably open/read back a serving-incarnation continuity guard bound to the current generation and one live authority. Only that authority's non-restorable capability may admit traffic; a missing, unsealed-orphaned, ambiguous, or unsealable guard quarantines authentication, viewer/history/publication, ingest, worker, export, persistence, callback, and ordinary/scheduled maintenance traffic. A trustworthy intact unmatched epoch permits only the sole mutually exclusive offline recovery actor to read authoritative recovery evidence, replay ingress, and attempt clean-close while all serving and credential-issuance paths remain isolated; missing, tampered, conflicting, or unreadable recovery evidence supplies no replay or close authority, so that actor halts without opening traffic. | Issues #4, #7, #16, #19 |
| `CTRL-SAFETY-GENERATION` | One monotonic generation orders an append-only journal and integrity-protected recovery snapshot containing an orthogonal global enable bit, complete canonical-room denylist, and latest per-room action provenance. Enable/remove first writes and reads back a predecessor-bound `PREPARED` proposal under an opaque proposal identity; preparation is not current and does not advance the generation or authorize traffic/recovery. Global enable conditionally promotes to `COMMITTED` before the same live incarnation's final non-restorable activation. Room removal instead leaves `R` in the current snapshot while the same live incarnation's final guard atomically mints/consumes a single-use non-restorable permit to dispatch exactly one conditional promotion; that promotion atomically removes `R`, with no fallible post-commit room effect. Guard/permit failure or process loss before dispatch leaves membership unchanged. After dispatch, an unknown result keeps recovery globally off until exact journal/head reconciliation proves either the matching permitted `COMMITTED` removal or an unchanged predecessor/`PREPARED` state; split or unverifiable state remains off. An ingest-independent guard family separates global `G` from room `Q[R]`: global disable increments `G`, revokes global activation, and starts all-room cleanup, while pending or committed `add(R)` increments only `Q[R]`, blocks/cleans `R`, invalidates only an unissued removal permit, and preserves unrelated-room activation. After dispatch, `Q[R]` is immediate while the competing exact-head CAS results reconcile. Scope-labelled pending entries prevent clean sealing but do not themselves widen runtime effect. A failed/ambiguous add invokes a distinct global-disable transition; after process loss, an unsealed/orphaned continuity guard globally quarantines recovery. A generation change reloads/re-evaluates state; it does not itself imply global cleanup. | Issues #4, #7, #16, #17, #19 |
| `CTRL-ROOM-DENYLIST` | Resolve aliases/URLs and active-resource bindings to canonical room ID before eligibility. A pending-in-deadline or successfully committed add blocks/cleans only the matching room and preserves unrelated rooms; add failure/ambiguity explicitly escalates through global disable. Global enable preserves the list and removing one room does not enable global ingest. A removal stays prepared with membership intact until the exact live incarnation passes the room guard and consumes its one-use permit to dispatch conditional promotion; a failed final guard, lost/prior-incarnation permit, or crash before dispatch cannot publish a snapshot without the room. After dispatch, recovery accepts absence only from the matching permitted `COMMITTED` head and otherwise retains the room or stays globally off. Ordinary offline/ended live status is transient: stop or reject only that start/reconnect, without adding the room, advancing the generation, or requiring owner-gated removal. | Issues #7, #16, #17 |
...
| `CTRL-RESTORE-REPLAY` | Reject restored stateful/stateless credentials; reject every prepared/ambiguous relaxation, every prior-incarnation removal permit, and any committed enable lacking a freshly reconciled current-incarnation activation capability; quarantine an orphaned/unsealed serving guard; reconcile every continuity failure against authoritative ingress replay, admitting each unresolved valid room/session intake to a verified replayable tombstone, resolving each valid account/device intake to its typed checkpoint, and reproducing durable denial for each invalid request; replay current tombstones/checkpoints; and reconcile the complete global/denylist snapshot before accepting any authentication, viewer/history/publication, ingest, worker, export, persistence, callback, or ordinary/scheduled maintenance traffic. A trustworthy unmatched epoch lets only the mutually exclusive offline actor continue replay and clean-close; an unpromoted removal still has its room in the snapshot. | Issues #4, #12, #13, #16, #19 |
| `CTRL-IDENTITY-RESTORE-REVOCATION` | Before account/device deletion or revocation intake, commit and read back the existing target-free `open(E)`. Accepting the request installs the existing pending fence under the same clean-close authority before local credential, issuance, enrollment, or worker authority is revoked and before the first target-specific recovery write. Valid intake resolves only to a commit/read-back-verified typed pseudonymous checkpoint outside application backups; invalid intake resolves only to durable denial. First-write/read-back failure, uncertainty, source loss, or crash leaves the epoch unsealable, rejects every pre-restore credential and new issuance, and requires authoritative replay rather than inference from a stale backup, empty checkpoint view, or audit. | Issues #4, #12, #13, #16, #17, #19 |
| `CTRL-REENABLE-GATE` | Admin technical global re-enable or exact-room removal only after remediation, current evidence, tabletop, recorded owner approval, authoritative replay of every unresolved deletion intake/continuity hazard, successful reconciliation or clean sealing of any prior serving-incarnation guard, and rejection of every prepared/split/ambiguous relaxation. Enable preserves the denylist and requires the fresh current incarnation's post-commit activation. Removal never enables global ingest or starts a room; it requires that incarnation's pre-promotion one-use permit, so failed final checks leave the room durably denied and only a verified permitted promotion removes it. | Issues #7, #16, #17, #19; repository owner decision |
...
| `CTRL-BACKUP-EVIDENCE` | Enumerate and evidence every cache, replica, object version, export, and backup window. Keep the current recovery-visible serving guard and every unresolved safety proposal/result outside application backups until reconciled; retain admitted room/session tombstones, unresolved room/session/account/device intake blockers, durable invalid denials, identity/device checkpoints, and independent auth-invalidation state through every applicable window and one verified post-window restore. Never claim physical erasure or persist a process-local activation capability. | Issues #4, #12, #13, #16, #19 |
| `CTRL-DELETION-FAIL-CLOSED` | Before verified admission, containment is provisional and no deletion state/success is reported or purge started. A pending valid room/session request blocks only its exact scope, an accepted account/device request blocks only its target authority, and a pending invalid request starts no guessed action; none revokes unrelated serving. Verified tombstone or typed-checkpoint admission continues the applicable block, while durable invalid/unauthorized denial clears pending. Failed, timed-out, response-unknown, or unreadable intake/tombstone/checkpoint/denial persistence, or inability to install or prove the selected scope's containment/ownership, marks the epoch `tainted`, revokes global activation, and prevents clean sealing. Initiating-source loss plus backend crash then leaves an orphaned guard that globally quarantines recovery until authoritative replay replays the observed request to the applicable tombstone, checkpoint, or denial. If proof cannot be recovered, quarantine persists. The guard is not `hidden`, a checkpoint, a fourth deletion state, an SLA clock, or purge authority. After admission, a failed store stays scoped while isolation remains proven; loss of that proof escalates globally, and unchecked completion is never reported. Volatile containment, an audit row, or an empty store is not durable outcome evidence. | Issues #11, #12, #13, #16, #17 |
...
| ID | Asset | Actor | Entry point | Preconditions | Preventive controls | Detection | Response | Control owner / dependency | Residual | Decision |
| --- | --- | --- | --- | --- | --- | --- | --- | --- | --- | --- |
| `THREAT-DELETION-INCOMPLETE` | `ASSET-DELETION`, `ASSET-NORMALIZED`, `ASSET-RAW`, `ASSET-IDENTITY` | `ACT-PROVIDER`, coding error, or `ACT-PRIVILEGED` | `ENTRY-DATA`, `ENTRY-DELETION-RESTORE` | A request is acknowledged, `hidden` reported, or purge started before tombstone commit/read-back; ordinary pending or a durable invalid denial incorrectly revokes unrelated serving; admission/denial or scoped-containment failure does not taint/revoke globally; recovery demands a valid selector triple for an invalid request; durable intake omits/resets the original request time; exact intake persistence fails and the initiating source plus backend then crash without a recovery-visible continuity blocker; account/device authority is locally revoked before a verified `open(E)` and pending fence exist, or its first checkpoint write/read-back fails and crash/source loss lets a stale backup recreate or reissue authority; a pre/post-commit crash or lost response loses/duplicates admission; an audit row, volatile block, coarse guard, empty tombstone view, or empty identity-checkpoint view is mistaken for a durable outcome; a selector is invalid/conflicting; room/session scope or dominance is wrong; a cascade fails; or late success erases the SLA breach. | `CTRL-DELETION-STATE`, `CTRL-DELETION-FAIL-CLOSED`, `CTRL-SAFETY-DEFAULT-OFF`, `CTRL-IDENTITY-RESTORE-REVOCATION`, `CTRL-BACKUP-EVIDENCE`, `CTRL-ROOM-DENYLIST`; a shared pre-armed `open(E)` covering room/session and account/device ingress, explicit pending-versus-tainted intake states, pending-before-target-effect and first-write ordering, sticky backend ownership after failed accounting/admission, atomic durable valid room/session intake triple, typed valid identity checkpoint, durable invalid denial, combined-ingress high-watermark clean-close, verified `hidden` admission barrier, authoritative selector, room dominance, exact-scope cascade, and commit/read-back verification. | Valid session/room, invalid-selector, valid account/device, and invalid-identity pending probes verify ordering and unrelated activation; intake/tombstone/checkpoint/denial first-write, timeout, unknown-result, read-back, and scoped-containment/ownership-proof failures verify unsealable epoch, atomic taint, and global revoke; valid-triple, typed-checkpoint, and invalid-denial authoritative replay; immutable-clock comparison; source-loss/backend-crash and orphaned-guard recovery against a stale backup; pre/post-commit crash and response-loss retry; audit/coarse-guard/empty-store negatives; selector, room-all-child, sibling non-deletion, dominance, store, identity, retry, and SLA reconciliation. | While room/session admission is pending, keep only exact-scope provisional containment, or no guessed containment for an invalid selector. Account/device pending is only continuity/clean-close/relaxation accounting until authorization and target validation succeed; only a valid action then revokes the exact target before its first checkpoint write, while invalid or unauthorized intake changes no target and resolves only to durable denial. Report no success/state and start no purge before the required outcome. Verified room/session admission preserves its exact-scope block, and a verified identity checkpoint preserves target revocation, without global re-enable. Failed/ambiguous accounting, tombstone/checkpoint/denial persistence, or loss of containment proof atomically taints the epoch, revokes global activation, and prevents clean-close. The live backend retries rather than relying solely on its source. If both continuities are lost, isolate serving and issuance while authoritative replay recovers a valid room/session request's same triple, a valid account/device request's exact checkpoint, or an invalid request's durable denial. Absent applicable proof, quarantine persists without inventing `hidden`, a checkpoint, clock, or destructive scope. After admission retry/re-enumerate exactly, never report unchecked completion, and preserve dominance and late-SLA evidence. | Identity/data/privacy/operations owners; Issues #12/#13/#16/#17 | **High**: lost admission/time, revived identity authority, wrong scope, provider failure, an omitted authority row, or an unknown copy may expose retained data, hide lateness, or delete unrelated data. | **NOT ACCEPTED**; production auth/persistence OFF; see decision register. |
| `THREAT-RESTORE-RESURRECTION` | `ASSET-DELETION`, `ASSET-SAFETY`, `ASSET-IDENTITY`, `ASSET-SECRET`, all persisted data | `ACT-PROVIDER`, mistaken `ACT-PRIVILEGED`, stale backup | `ENTRY-DELETION-RESTORE`, `ENTRY-MAINTENANCE` | Restart/restore treats an empty application store/audit/checkpoint view as proof, trusts a prepared/ambiguous relaxation or committed enabled snapshot without a fresh current-incarnation activation capability, restores a guard-failed or pre-dispatch room removal as an absent entry, guesses an unknown post-dispatch result, or reuses its prior-incarnation permit; seals or ignores an orphaned continuity guard; opens with unresolved or exact-intake-lost room/session/account/device scope; fails tombstone/checkpoint/denial replay; recreates or issues authority to a deleted/revoked identity; or, conversely, terminates recovery on a trustworthy unmatched epoch before authoritative replay and clean-close can resolve it; loses room dominance; accepts a conflicting parent map or pre-restore credential/key; uses stale safety scope or an unbounded backup/object window; or proceeds despite missing/tampered/conflicting recovery evidence. | `CTRL-SAFETY-DEFAULT-OFF`, `CTRL-RESTORE-REPLAY`, `CTRL-DELETION-STATE`, `CTRL-IDENTITY-RESTORE-REVOCATION`, `CTRL-AUTH-KEY-LIFECYCLE`, `CTRL-BACKUP-EVIDENCE`, `CTRL-MAINTENANCE-EXCLUSIVE`; process-bound non-restorable global activation and removal permit, combined-ingress continuity-guard quarantine, mutually exclusive offline replay, durable typed outcomes, and provider-window/integrity evidence. | Orphaned/unsealed-guard, exact-intake-loss, identity first-write/read-back/source-loss, and trustworthy-unmatched-epoch continuation restart probes; missing/tampered/conflicting recovery-copy halt negatives; global-enable prepared/post-promotion/pre-activation and committed-without-capability crash probes; room-removal final-guard/permit failure, crash before dispatch, crash after dispatch with unknown outcome, and post-promotion response-loss probes; unresolved-intake and missing tombstone/checkpoint/denial restore negatives; offline credential/new-authority rejection; room/session/sibling/dominance reconciliation; auth and safety-snapshot generation comparison; combined-high-watermark clean-close and post-window verification. | Keep every ordinary/external/scheduled traffic class and credential issuance isolated/off. For a trustworthy intact unmatched epoch, permit only the sole mutually exclusive offline recovery actor to continue authoritative ingress replay and resolve valid room/session intake to verified tombstones, valid account/device intake to verified typed checkpoints, and invalid intake to durable denial. While still isolated, reject restored credentials, advance protected auth state, apply exact-scope tombstones/checkpoints and issuance denial, and reconcile prepared/split safety transitions plus the current safety head. Only after every combined-high-watermark outcome is durable and applied may the actor quiesce ingress and commit/read back `clean-close(E)` for the old epoch; it must then commit/read back a fresh `open(E)` before any fresh non-restored audited recovery-admin activation. If authoritative replay or close cannot resolve the epoch, keep isolation indefinitely; if recovery evidence is missing, tampered, conflicting, or unreadable, it supplies no replay/seal authority, so the offline actor halts without opening traffic. Never reuse a prior incarnation's activation or removal permit, guess a purge/checkpoint, accept unknown room-removal state, or allow a stale backup to restore authority. | Identity/data/operations owners; Issues #4/#12/#13/#16/#19 | **High**: an omitted/unadmitted/wrong-scope target, prematurely abandoned replay, orphaned authority epoch, stale enabled snapshot, failed removal restored as absent, or accepted pre-restore credential can republish data, delete unrelated data, or recreate authority. | **NOT ACCEPTED**; auth/persistence/recovery production paths OFF; see decision register. |
| `THREAT-SAFETY-ROLLBACK` | `ASSET-SAFETY`, `ASSET-AVAILABILITY` | Compromised/mistaken `ACT-PRIVILEGED`, stale backup, storage failure | `ENTRY-BROWSER` admin control, `ENTRY-DATA`, `ENTRY-DELETION-RESTORE` | A newer global/room decision is overwritten; global cleanup waits behind slow/hung durable I/O; generic tightening state lets pending `add(R)` revoke unrelated activation; a failed add never invokes global disable; global enable is treated as serving before its final activation; room removal drops `R` from the durable snapshot before its final guard/permit succeeds; a failed removal guard is lost on restart and later global enable opens `R`; tightening exists only in volatile state when the process crashes; a delayed enable/remove response clears a newer `G`/`Q[R]`; multiple active owners do not share/acknowledge the guards; a crash, split journal/recovery commit, or lost response drops scope/state; clean-close races a late safety action; enable drops denylist entries or removal enables global ingest; canonical/resource binding is ambiguous; state is stale/lost; or relaxation lacks owner approval. | `CTRL-SAFETY-DEFAULT-OFF`, `CTRL-SAFETY-GENERATION`, `CTRL-ROOM-DENYLIST`, `CTRL-DISABLE-CLEANUP`, `CTRL-REENABLE-GATE`; one active Alpha serving authority, pre-armed continuity guard, process-bound non-restorable global activation and one-use room-removal permit, non-current predecessor-bound prepared relaxation, future-owner isolation on guard uncertainty, distinct high-priority `G` and `Q[R]`, pre-durability scope-correct cleanup, one-generation orthogonal snapshot, predecessor binding, and explicit add-failure escalation. | Hold exact `add(B)` pending within deadline while `A` remains active and prove `G`/`A` resources unchanged; then inject timeout/unknown/read-back failure and prove a separately identified global-disable first changes `G` and starts all-room cleanup. For removal, inject final-guard failure, permit loss, prior-incarnation permit, crash before dispatch, crash after dispatch with an unknown result, add/intake before and after dispatch, both exact-head CAS winners, and response loss after verified promotion; prove `R` remains after every pre-dispatch failure, an unknown post-dispatch outcome remains globally off pending exact reconciliation, and fresh global enable cannot bypass either case. Also test active-owner guard acknowledgement; both linearization orders for global enable versus tightening and disable versus add; clean-close races; pre-/mid-/post-promotion and activation crash/response loss; orphaned-guard recovery; generation mismatch; global-enable/room-remove noninterference; stale/concurrent operation and binding/integrity alerts. | Apply exact scope without awaiting durability: global disable increments `G`, revokes global activation, and starts all-room cleanup; pending/committed `add(R)` increments only `Q[R]`, blocks/cleans `R`, and preserves unrelated activation. Add timeout/unknown/binding or commit/read-back failure retains the safe room block where exact and invokes a separate global-disable transition; only that action changes `G` and expands cleanup. Global enable promotes before current-incarnation activation, so a committed candidate alone admits no traffic. Room removal keeps membership until its final guard atomically mints/consumes a one-use permit to dispatch conditional promotion. Guard/permit/process loss before dispatch leaves membership unchanged. After dispatch, `G`/`Q[R]` or a deletion selector block applies immediately; if removal wins the exact-head CAS, safety tightening replays against the promoted head, while a same-head safety winner makes removal fail without rebasing and retains `R`. Unknown post-dispatch state remains globally off until exact reconciliation. Process loss globally quarantines unresolved recovery under the orphaned guard. Isolate/terminate an unacknowledged future owner and reconcile split/unknown state without dropping the denylist. | Safety/security/operations owners; Issues #7/#16/#17/#19 | **High**: delayed cleanup, stale relaxation, uncoordinated authority, lost/split state, control compromise, or wrong scope can expose denied content or cause broad availability loss. | **NOT ACCEPTED**; production ingest OFF; see decision register. |
...
| `THREAT-DELETION-INCOMPLETE` | Production normalized/raw/identity persistence and managed exports subject to room/session/account/device deletion or revocation. | High | `CTRL-DELETION-STATE`, `CTRL-DELETION-FAIL-CLOSED`, `CTRL-SAFETY-DEFAULT-OFF`, `CTRL-IDENTITY-RESTORE-REVOCATION`, `CTRL-BACKUP-EVIDENCE`, provider inventory/window evidence, shared-`open(E)` and combined-close proof, pending-before-identity-revocation, tombstone/checkpoint/denial commit-read-back, first-write failure plus source/backend-crash continuity tests, invalid-selector, room-all-child, session-sibling, dominance, cascade/read-back, and partial-failure tests. | Admin and identity/data/privacy owner | **NOT ACCEPTED — approver/date/review date PENDING; auth/persistence OFF** |
| `THREAT-RESTORE-RESURRECTION` | Restoring any production data backup or object-version state, including unresolved/admitted or exact-intake-lost room/session/account/device scope, orphaned serving-incarnation state, identity, and stateful/stateless credential state. | High | `CTRL-SAFETY-DEFAULT-OFF`, `CTRL-RESTORE-REPLAY`, `CTRL-DELETION-STATE`, `CTRL-IDENTITY-RESTORE-REVOCATION`, `CTRL-AUTH-KEY-LIFECYCLE`, `CTRL-BACKUP-EVIDENCE`, continuity-guard quarantine, trustworthy-unmatched offline-progress and corrupted-copy halt tests, unresolved-intake blocking, prepared/ambiguous-relaxation rejection, current-incarnation global-activation proof, rejection of prior-incarnation room-removal permits, tombstone/checkpoint/denial replay, combined-high-watermark clean-close, exact-scope/dominance restore, credential rejection, and deleted-authority issuance probes. | Identity/operations/data owner | **NOT ACCEPTED — approver/date/review date PENDING; auth/recovery path OFF** |
| `THREAT-SAFETY-ROLLBACK` | Production global enablement, room-scoped denylist transitions, and restored safety state. | High | `CTRL-SAFETY-DEFAULT-OFF`, `CTRL-SAFETY-GENERATION`, `CTRL-ROOM-DENYLIST`, `CTRL-DISABLE-CLEANUP`, `CTRL-RESTORE-REPLAY`, `CTRL-REENABLE-GATE`, independent recovery copy, global-enable commit/final-activation tests, room-removal final-guard/one-use-permit/promotion tests, orphaned-guard quarantine, exact-scope/noninterference/concurrency tests, and rollback/unauthorized-reenable tabletop. | Operator/admin for immediate disable; safety owner | **NOT ACCEPTED — approver/date/review date PENDING; ingest OFF** |
...
2. One generation orders an orthogonal global-enable bit and complete denylist. Global
   ...
   recovery-effective. Global enable conditionally promotes first, but even its
   `COMMITTED` snapshot cannot admit traffic without the same live serving incarnation's
   non-restorable capability minted by the atomic final local guard check-and-effect.
   Room removal instead keeps membership current through preparation and uses one atomic
   final local guard operation to mint/consume a one-use non-restorable permit and dispatch
   the conditional promotion that atomically removes the room. Global disable increments `G` and preempts slow relaxation I/O
   by revoking that capability; `add(R)` increments only `Q[R]`, invalidates only an unissued
   matching room-removal permit, and keeps unrelated rooms' global activation. After dispatch,
   `G`/`Q[R]` takes immediate effect while the competing exact-head CAS outcomes reconcile:
   a removal-winning CAS requires tightening replay against the promoted head, while a
   safety-tightening winner makes removal fail without rebasing and keeps the room. Every scope-labelled
   pending handler prevents clean sealing, but pending alone does not widen runtime scope.
   A prepared, split, ambiguous, global-enable post-promotion/pre-activation, or room-
   removal guard/permit-failed relaxation stays non-effective; a failed room removal before
   dispatch leaves membership unchanged, while an unknown post-dispatch result stays globally
   off until exact head reconciliation. After process loss an unsealed/orphaned continuity guard
   prevents unresolved state from reopening. Canonical/resource ambiguity, stale/conflicting
...
3. A deletion request first enters `intake-pending` and validates exactly one authoritative
   ...
   substitute. The same verified target-free `open(E)` and close authority cover account/
   device deletion or revocation intake. That path installs pending before local target
   authority is revoked or its first target-specific recovery write begins; valid intake
   resolves to a commit/read-back-verified typed checkpoint, invalid intake resolves to
   durable denial, and first-write/read-back failure or crash leaves the epoch unsealable
   and issuance blocked pending authoritative replay. The epoch alone never invents a
   checkpoint or identity operation.
...
5. Every restart/restore has no reusable activation capability and keeps ordinary/external/
   scheduled traffic plus credential issuance globally quarantined while a prior serving
   guard is unsealed/orphaned, any intake continuity is unproven, or any relaxation is
   prepared/split/ambiguous. A trustworthy intact unmatched epoch does not terminate the
   recovery procedure: only the sole mutually exclusive offline recovery actor may continue
   authoritative ingress replay while all serving paths remain isolated. A missing,
   tampered, conflicting, or unreadable recovery
   copy supplies no replay/seal authority, so the offline actor halts without opening
   traffic. Before a fresh capability may be minted, every
   unresolved valid room/session intake must become a verified replayed tombstone, every
   valid account/device intake a verified typed checkpoint, and every invalid intake a
   durable denial; every restored stateful credential row must be purged/revoked; protected
   auth state must reject every stateless pre-restore credential; exact-scope/dominance
   proofs and the complete safety snapshot must reconcile. Only after all of those outcomes
   are durable and applied may the actor clean-close the old epoch; it must then commit/read
   back the new incarnation's `open(E)` before any fresh activation. A backup, `COMMITTED` enabled
   snapshot alone, empty application/checkpoint store, or audit row cannot replace recovery
   evidence, restore a prior capability/current key material, or recreate deleted authority.
...
7. Re-enable or exact-room denylist removal requires cause remediation, current platform/
   rights evidence, authoritative replay of every room/session/account/device deletion or
   revocation intake, verified combined-ingress clean-close, reconciliation of every serving-
   incarnation continuity hazard, rejection of every prepared/split/ambiguous relaxation, successful
...
- deletion tests prove the recovery-visible serving guard is durably open before either
  room/session or account/device deletion/revocation ingress accepts a request;
...
- account/device deletion/revocation tests hold intake at each boundary and prove the
  existing pending fence linearizes before local credential/session/enrollment/worker-
  authority revocation and before the first target-specific recovery write; valid intake
  resolves only to a commit/read-back-verified typed checkpoint, invalid intake only to
  durable denial, and neither target-free `open(E)` nor an empty checkpoint/audit view is
  accepted as a result. Inject first checkpoint write/read-back failure, timeout, unknown
  result, process crash, and initiating-source loss against a stale backup; prove the epoch
  cannot cleanly seal, ordinary/external/scheduled traffic and old/new credential issuance
  stay isolated, and no deleted/revoked authority is recreated. The restore tabletop must
  then prove a trustworthy intact unmatched epoch allows only the mutually exclusive
  offline recovery actor to replay a valid identity request to its exact checkpoint or an
  invalid request to durable denial and close only after the combined safety, room/session,
  and account/device high-watermark is reconciled; replay loss keeps isolation indefinitely,
  while missing/tampered/conflicting recovery evidence halts the offline actor and provides
  no replay or close authority;
- safety tests distinguish global `G` from matching and unrelated-room `Q[R]`; hold
  ...
  incarnation. Prove room removal instead leaves `R` present through `PREPARED` and uses
  one atomic final guard to mint/consume a one-use permit and issue conditional promotion,
  with no fallible post-commit effect; inject
  disable versus enable and add/intake versus remove before preparation, before/after
  atomic permit mint/consumption and around promotion; specifically
  ...
  fail the room-removal final guard, lose/reuse a permit, and crash before promotion dispatch,
  then perform fresh global enable and prove `R` remains durably denied; separately crash
  after dispatch with the outcome unknown and prove global-off exact-head reconciliation
  distinguishes matching `COMMITTED`, unchanged predecessor/`PREPARED`, and split/unknown
  state; race add/global-disable with the dispatched CAS and prove either removal wins followed
  by tightening replay or tightening wins and removal fails without rebasing; then lose the
  response after permitted promotion and prove idempotent success without a second removal;
...
- invalid room/session selector rejection, room-all-child and session-sibling exact-scope
  ...
  identity/device checkpoint-or-denial replay, combined-ingress clean-close, trustworthy-
  unmatched offline continuation, hard recovery-copy integrity failure, and fresh recovery-
  admin paths have negative tests or tabletop evidence appropriate to their Issue;
